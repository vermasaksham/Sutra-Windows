...
src/
  ...
  notes/
    ...
    AiPanel.tsx       Drafts, marked as generated and inert until accepted
    AiSettings.tsx    Switching it on, and what leaves the machine
...
src-tauri/
  src/
    ...
    ai.rs             The optional assistant, and the rules it cannot break.
...
Four rules, each structural rather than a promise in a prompt:

**It may not write to a file.** `ai.rs` does not import `vault`, which is the
only module that writes. An `Assistant` returns a `Draft` — a value with no
path and no id, and nothing the frontend could hand back that would make it
write. Accepting a draft calls the same `save_note` or `set_note_meta` that
typing goes through, so generated text has no privileges over typed text and
the app has exactly one road to a file.

**It may not cite what the vault does not have.** Every response is run through
`citations::drop_unknown` against the vault's real source ids; an invented
`[@ref]` is removed and the removal is reported on screen. The prompt asks too,
...
**Marked as generated.** A draft renders on its own ground, in a dashed border,
headed `GENERATED BY <model> · NOT SAVED`. It must not be possible to read the
panel at a glance and think the note says it.

### What leaves the machine

The one note you asked about — title and body — and, for a tag suggestion, the
list of tags the vault already uses. Nothing else: no other notes, no folder
paths, no attachments. The whole of it is one `Ask` struct, so the answer to
"what did it send" is a type rather than an audit.

The default model is Claude Opus 5, over its HTTP API from Rust — the frontend
never sees a key. Thinking is adaptive on that model by default, so depth is
steered with `effort` per task instead: `low` for tags, which is classification
against a list already in the prompt, `medium` for the two reading tasks.
Refusal fallbacks are on (`fallbacks: "default"`), because without them a
declined request just stops, which in a notes app reads as the feature being
broken rather than as a decision.

The API key is read from `ANTHROPIC_API_KEY` first, and only then from Sutra's
config file — where it sits in **plain text**, readable by anything running as
you. That is said in the settings dialog too, along with the environment
variable as the alternative that stores nothing at all. A real secret store is
worth having and is not in this step. The key is never written into the vault:
a vault gets synced, backed up and shared, and a credential does not belong
somewhere that happens to.

**Proof:** with the assistant answering every question and every draft
rejected, the vault is byte-identical — asserted the way saved views and the
context panel are, by hashing every markdown file before and after. Rejecting
is not an operation at all; there is nothing to undo, because asking never
wrote anything.
...
views, the context panel, duplicate candidates, numeric claims that differ, and
optional AI assistance that is off until switched on.
